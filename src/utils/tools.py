...
OUT_DIR = PROJECT_DIR / "out_mnist_niid2_fixedclientlabel_trial1"
    ...
    if isinstance(src, OrderedDict):
        for name, param in src.items():
            if param.requires_grad:
                parameters.append(func(param) if not cpu else func[param].cpu())
                keys.append(name)
    elif isinstance(src, torch.nn.Module):
        for name, param in src.state_dict(keep_vars=True).items():
            if param.requires_grad and (True if personal_tag is None else personal_tag not in name):
                adapted_p = func(param) if not cpu else func(param).cpu()
                parameters.append(adapted_p)
                keys.append(name)
...
def evalutate_model(
    ...
    for x, y in tqdm(dataloader):
        # print(f'inst_x shape: {inst_x.shape}')
        ...
        # res = model(x, y)
        # loss += model.compute_loss(res).item() * len(y)
        loss += model.valid_step(x, y) * len(y)
        # loss += criterion(logits, y).item()
        # pred = torch.argmax(logits, -1)
        # correct += (pred == y).sum().item()
        correct = 0
        sample_num += len(y)
    return loss / sample_num, correct, sample_num
