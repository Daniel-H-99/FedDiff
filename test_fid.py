...
true_image_dir = '/home/server33/minyeong_workspace/FL-bench/data/cifar10/raw'

CID=0
def init_wandb():
    wandb.init(project='fids', name=f'pretrained_50000vs50000_cifar10_client')
...
def calc_fid_dict(checkpoints):
    ...
    for ckpt in checkpoints:
        ...
        for client_id in range(CID, CID + 1):
            ...
            syn_all_path = os.path.join(image_fid_dir, f'{epoch}', 'local', f'{client_id}')
            ...
            res[f'local_local_client_{client_id}'] = calc_fid(syn_all_path, true_local_path)
            # res[f'local_global_client_{client_id}'] = calc_fid(syn_local_path, true_global_path)
            # res[f'global_global_client_{client_id}'] = calc_fid(syn_local_path, true_global_path)
            # res[f'global_global_client_{client_id}'] = calc_fid(syn_global_path, true_global_path)
...
def main():
    ...
    ckpt_dir = f'/home/server33/minyeong_workspace/FL-bench/out_cifar10_fed_trial1/FedDiff/checkpoints'
    files = sorted(list(set([int(f.split('_')[1]) for f in os.listdir(ckpt_dir) ])))
    ckpt_name_list = [os.path.join(ckpt_dir, f"cifar10_{f}_custom") for f in files if (f > 9) and (f < 12)]
