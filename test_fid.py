...
image_fid_dir = '/home/server36/minyeong_workspace/FL-bench/out_organa_niid_localcode_trial1/FedDiff/images_fid'
true_image_dir = '/home/server36/minyeong_workspace/FL-bench/data/organa_niid/raw'

CID=0
def init_wandb():
    wandb.init(project='v2_fid', name=f'localcode_organa_niid_client{CID}')
...
def calc_privacy2(path_1, path_2, idx_1=None, idx_2=None, ret_idx=False):
...
def main():
    ...
    ckpt_dir = f'/home/server36/minyeong_workspace/FL-bench/out_organa_niid_localcode_trial1/FedDiff/checkpoints'
    files = sorted(list(set([int(f.split('_')[2]) for f in os.listdir(ckpt_dir) ])))
    ckpt_name_list = [os.path.join(ckpt_dir, f"organa_niid_{f}_custom") for f in files if (f > 180) and (f <= 200)]
    ...
    # with open(f'tested_fid_localcode_organa_niid_client_{CID}.pkl', 'wb') as f:
    #     pkl.dump(fid_dict, f)
    ...
    # with open(f'tested_privacy2_localcode218_organa_niid_client_{CID}.pkl', 'wb') as f:
