...
def main():
    ...
    # print(f'total data: {num_samples_to_agg.sum()}')
    # while True:
    #     continue
    for cid in cid_list:
        ...
        #     imwrite(save_path, data[idx])
        all_train_idx.append(np.random.permutation(train_idx_list)[:int(0.9 * num_samples_to_agg[cid])])
        all_test_idx.append(np.random.permutation(test_idx_list)[:num_samples_to_agg[cid] - int(0.9 * num_samples_to_agg[cid])])
