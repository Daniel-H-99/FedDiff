...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(f'min, max, var: {data.min()} vs {data.max()} vs {data.var()}')
        # while True:
        #     continue
        return data, targets
...
class InSTBaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        data, targets = self.data[index], self.targets[index]
        img = copy.deepcopy(data).permute(1, 2, 0).numpy().astype(np.uint8)
        if self.enable_train_transform and self.train_data_transform is not None:
            data = self.train_data_transform(data)
        if self.enable_train_transform and self.train_target_transform is not None:
            targets = self.train_target_transform(targets)
...
class RawPathMNIST():
    def __init__(self, split='all', path='/home/server40/minyeong_workspace/vae/data/pathmnist/pathmnist.npz'):
        self.mean = np.array([0.73971496, 0.53328612, 0.70616016])
        self.sig = np.array([0.12538848, 0.17919966, 0.12639665])
        self.split = split
        self.path = path
        ...
        for sp in sps:
            images.append(data[f'{sp}_images'])
            labels.append(data[f'{sp}_labels'])
        # print(f'img shape: {[im.shape for im in images[:10]]}')
    ...
    def __getitem__(self, idx):
        ...
        img = (img - self.mean[None, None]) / self.sig[None]
...
class InSTPathMNIST(InSTBaseDataset):
    def __init__(
        self,
        root,
        args=None,
